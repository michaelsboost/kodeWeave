...
function enter(syntaxTree, from, to, typeName, doc) {
    var _a;
    switch (typeName) {
        case 'AttributeValue': {
            ...
            overlayTree.iterate({
                ...
                enter: ({ type, from: overlayFrom, to: overlayTo }) => {
                    const maybeWidgetOptions = enter(syntaxTree, 
...
function colorPickersDecorations(view$1) {
    ...
    for (const range of view$1.visibleRanges) {
        st.iterate({
            ...
            to: range.to,
            enter: ({ type, from, to }) => {
                const maybeWidgetOptions = enter(st, from, to, type.name, view$1.state.doc);
                if (!maybeWidgetOptions) {
                    return;
                }
                if (!Array.isArray(maybeWidgetOptions)) {

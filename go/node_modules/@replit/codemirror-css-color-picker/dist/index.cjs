...
function colorPickersDecorations(view$1) {
    const widgets = [];
        ...
        language.syntaxTree(view$1.state).iterate({
            ...
            enter: ({ type, from, to }) => {
                if (type.name === 'CallExpression') {
                    const callExp = view$1.state.doc.sliceString(from, to);
                    if (callExp.startsWith('rgb')) {
                        ...
                        const widget = view.Decoration.widget({
                ...
                if (type.name === 'ColorLiteral') {
                    const [color, alpha] = toFullHex(view$1.state.doc.sliceString(from, to));
                    const widget = view.Decoration.widget({
                        widget: new ColorPickerWidget({
                ...
                if (type.name === 'ValueName') {
                    const colorName = view$1.state.doc.sliceString(from, to);
...
const colorPickerViewPlugin = view.ViewPlugin.fromClass(class ColorPickerViewPlugin {
    constructor(view) {
